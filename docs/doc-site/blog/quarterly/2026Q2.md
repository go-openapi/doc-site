...
weight: 8
discord_description: |-
  A quarter of focused, feature-driven work across go-openapi **and** go-swagger:

  • **runtime** — package-layout refactor, a new standalone `server-middleware` module reusable in any `net/http` server, a curl-style connection diagnostic, and a security pass (public API preserved)
  • **codescan** — spun off from go-swagger and refactored; a cautious rewrite to retire its 60–70 regexes is underway
  • **go-swagger** — back to active releases (**v0.34.0**) via goreleaser; bundled examples moved to their own repo
  • **jsonpointer** is now feature-complete; **swag** has smarter JSON naming; **strfmt** has faster duration parsing
  • Organization-wide docs consolidated on the new doc-site

  🙏 Thanks to this quarter's contributors: @KT-Doan, @alexandear, @KuaaMU, @boekkooi-impossiblecloud, @dashitongzhi, @Abzaek.

  Apr 1 – May 29, 2026 · 352 commits across 18 repositories
---
